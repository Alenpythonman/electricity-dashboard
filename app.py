...
start_date = st.sidebar.date_input("Start date", min_date, key="start_date_filter")
end_date = st.sidebar.date_input("End date", max_date, key="end_date_filter")
...
st.caption("Half hourly electricity analysis for electrical engineering assessment, battery sizing, and screening level commercial review")
...
p1.metric("Base Load", f"{base_load_kw:,.1f} kW")
p2.metric("P50", f"{p50_kw:,.1f} kW")
p3.metric("P90", f"{p90_kw:,.1f} kW")
p4.metric("P95", f"{p95_kw:,.1f} kW")
p5.metric("P99", f"{p99_kw:,.1f} kW")

st.markdown(f"**Peak interval timestamp:** {peak_time}")
...
threshold_kw = st.number_input("Demand threshold (kW)", min_value=0.0, value=float(round(p95_kw, 1)), step=10.0, key="threshold_kw")
...
with bs1:
    target_limit_kw = st.number_input("Target demand limit (kW)", min_value=0.0, value=float(round(p95_kw, 1)), step=10.0, key="target_limit_kw")

with bs2:
    round_trip_efficiency = st.number_input("Round trip efficiency (%)", min_value=50.0, max_value=100.0, value=90.0, step=1.0, key="round_trip_efficiency")

with bs3:
    usable_dod = st.number_input("Usable depth of discharge (%)", min_value=10.0, max_value=100.0, value=90.0, step=1.0, key="usable_dod")

with bs4:
    max_discharge_duration_hours = st.number_input("Minimum discharge duration to support (hours)", min_value=0.5, max_value=8.0, value=1.0, step=0.5, key="max_discharge_duration_hours")
...
st.markdown("Enter commercial assumptions to estimate annual savings, battery capex, and simple payback.")

s1, s2, s3 = st.columns(3)

with s1:
    energy_price_per_kwh = st.number_input("Energy price (£/kWh)", min_value=0.0, value=0.15, step=0.01, key="energy_price_per_kwh")

with s2:
    demand_charge_per_kw_per_month = st.number_input("Demand charge (£/kW/month)", min_value=0.0, value=10.0, step=1.0, key="demand_charge_per_kw_per_month")

with s3:
    annualisation_mode = st.selectbox(
        "Annualisation basis",
        ["Scale selected period to 365 days", "Use selected period only"],
        key="annualisation_mode"
    )

s4, s5, s6 = st.columns(3)

with s4:
    battery_cost_per_kwh = st.number_input("Battery cost (£/kWh)", min_value=0.0, value=250.0, step=10.0, key="battery_cost_per_kwh")

with s5:
    battery_cost_per_kw = st.number_input("Battery cost (£/kW)", min_value=0.0, value=150.0, step=10.0, key="battery_cost_per_kw")

with s6:
    project_life_years = st.number_input("Project life (years)", min_value=1, value=10, step=1, key="project_life_years")

if annualisation_mode == "Scale selected period to 365 days":
    annualisation_factor = 365 / num_days if num_days > 0 else 0
else:
    annualisation_factor = 1

annual_raw_shaved_energy_kwh = raw_peak_shaving_energy_kwh * annualisation_factor
annual_energy_savings_gbp = annual_raw_shaved_energy_kwh * energy_price_per_kwh

annual_demand_savings_gbp = peak_reduction_kw * demand_charge_per_kw_per_month * 12

total_annual_savings_gbp = annual_energy_savings_gbp + annual_demand_savings_gbp

estimated_battery_capex_gbp = (
    recommended_battery_capacity_kwh * battery_cost_per_kwh
    + required_battery_power_kw * battery_cost_per_kw
)

simple_payback_years = (
    estimated_battery_capex_gbp / total_annual_savings_gbp
    if total_annual_savings_gbp > 0 else 0
)

simple_roi_percent = (
    (total_annual_savings_gbp / estimated_battery_capex_gbp) * 100
    if estimated_battery_capex_gbp > 0 else 0
)

r1, r2, r3, r4 = st.columns(4)
r1.metric("Annual Shaved Energy", f"{annual_raw_shaved_energy_kwh:,.0f} kWh")
r2.metric("Annual Energy Savings", f"£{annual_energy_savings_gbp:,.0f}")
r3.metric("Annual Demand Savings", f"£{annual_demand_savings_gbp:,.0f}")
r4.metric("Total Annual Savings", f"£{total_annual_savings_gbp:,.0f}")

r5, r6, r7 = st.columns(3)
r5.metric("Estimated Battery Capex", f"£{estimated_battery_capex_gbp:,.0f}")
r6.metric("Simple Payback", f"{simple_payback_years:,.1f} years")
r7.metric("Simple ROI", f"{simple_roi_percent:,.1f}%")

st.caption(
    "This is a screening level commercial estimate. It does not yet include charging energy cost, degradation, financing, O&M, augmentation, or detailed dispatch optimisation."
)
...
st.markdown(f"""
- **P95 demand** is **{p95_kw:,.1f} kW**, which is a strong starting point for a practical peak shaving threshold.
- **P99 demand** is **{p99_kw:,.1f} kW**, showing the level of more extreme but less frequent peaks.
- **Peak demand** is **{peak_demand_kw:,.1f} kW**, so compare this with P95 and P99 to judge whether the site has occasional spikes or sustained high demand.
...
- The current screening estimate indicates a recommended battery size of **{recommended_battery_capacity_kwh:,.1f} kWh** with **{required_battery_power_kw:,.1f} kW** discharge capability.
- The current commercial estimate suggests **£{total_annual_savings_gbp:,.0f}** annual savings and a simple payback of **{simple_payback_years:,.1f} years**.
""")
...
- Estimated annual energy savings are **£{annual_energy_savings_gbp:,.0f}**.
- Estimated annual demand charge savings are **£{annual_demand_savings_gbp:,.0f}**.
- Total estimated annual savings are **£{total_annual_savings_gbp:,.0f}**.
- Estimated battery capital cost is **£{estimated_battery_capex_gbp:,.0f}**, giving a simple payback of **{simple_payback_years:,.1f} years** and a simple ROI of **{simple_roi_percent:,.1f}%**.

**Engineering note**
- This dashboard provides a screening level assessment for battery selection and peak shaving suitability.
- Final battery specification should be confirmed using a detailed dispatch study including tariff structure, charge strategy, state of charge limits, efficiency losses, degradation, and operational constraints.
"""
...
Total energy consumption: {total_energy_kwh:,.0f} kWh
Average daily energy: {avg_daily_energy_kwh:,.0f} kWh/day
Average demand: {avg_demand_kw:,.1f} kW
Peak demand: {peak_demand_kw:,.1f} kW
Peak timestamp: {peak_time}
...
Weekday average demand: {weekday_mean_kw:,.1f} kW
Weekend average demand: {weekend_mean_kw:,.1f} kW

Battery screening:
Target demand limit = {target_limit_kw:,.1f} kW
Peak reduction potential = {peak_reduction_kw:,.1f} kW
Required battery power = {required_battery_power_kw:,.1f} kW
Raw shaved energy = {raw_peak_shaving_energy_kwh:,.1f} kWh
Efficiency adjusted energy = {usable_energy_required_kwh:,.1f} kWh
Installed battery capacity = {installed_battery_capacity_kwh:,.1f} kWh
Recommended battery capacity = {recommended_battery_capacity_kwh:,.1f} kWh

Commercial screening:
Annual energy savings = £{annual_energy_savings_gbp:,.0f}
Annual demand savings = £{annual_demand_savings_gbp:,.0f}
Total annual savings = £{total_annual_savings_gbp:,.0f}
Estimated battery capex = £{estimated_battery_capex_gbp:,.0f}
Simple payback = {simple_payback_years:,.1f} years
Simple ROI = {simple_roi_percent:,.1f}%

Engineering note:
This is a screening level battery sizing and commercial estimate and should be followed by a full dispatch and commercial assessment.
"""

st.text_area("Copyable Report Summary", summary_text, height=350)
